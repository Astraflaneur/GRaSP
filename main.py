...
from grasp.pruning import PruningManager
from grasp.pruning.tracing import trace
from grasp.pruning.pruner import get_tensor_to_module_metadata
from grasp.masking.importance import Magnitude, LAMP, HRank, FPGM
from grasp.masking.mask import MaskingManager
...
HEURISTICS = {
    'l1': partial(Magnitude, p=1),  # L1 norm pruning: Prunes filters/weights based on their L1 norm (sum of absolute values).
    'l2': Magnitude,                # L2 norm pruning: Prunes filters/weights based on their L2 norm (magnitude/Euclidean distance).
    'lamp': LAMP,                   # LAMP pruning: Layer-Adaptive Magnitude Pruning, adjusts pruning sensitivity per layer using LAMP scores.
    'fpgm': FPGM,                   # FPGM pruning: Filters are pruned based on geometric median, preserving overall structure while removing redundant filters.
    'hrank': HRank,                 # HRank pruning: Prunes filters with low-rank feature maps, assuming they carry less information.
}
